...
<!--
上幾個章節，我們介紹了觀察 Re-render 的方式，包括 React Dev Tools 以及超好用的函式庫 React Scan

這個章節，我們就要正式進入到 React 優化的內容啦！
-->
...
這種情況下，就可以發現他會有很明顯的延遲。

那怎麼解決這個問題呢?

也就是我們的一個要談的優化技巧 — 狀態下移。
-->
...
<!--
這就是我們今天要談的第一種 React 優化方式：狀態下移，這種優化方式非常簡單也非常實用。
...
- 為什麼切換 `UserDetails` 會讓 `AnalyticsComponent` 與 `ChartComponent` 變慢？
- 「狀態下移」要解決的核心問題是什麼？
- 什麼情況下，state 不適合放在最外層 component？
...
- 在團隊中實際找一個「點一下就有延遲」的互動，檢查該 state 是否被放在過於外層的 component，並嘗試用狀態下移重構。
