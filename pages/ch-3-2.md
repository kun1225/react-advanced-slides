...
這個章節，我們就要來介紹常見觸發 Re-render 的場景，在了解之後，未來開發就能更避免效能上的問題。
-->
...
[click]
5. 當呼叫 setState，會去計算新的值並寫入對應的 hooks[index] 中，然後觸發一次 scheduleRender，讓整個 component 重新執行。
...
Context 是 React 提供的狀態共享機制，<span v-mark="{color: 'var(--secondary)'}">讓我們可以在不透過 props 傳遞的情況下，讓深層子組件存取狀態。</span>
...
Context 是 React 提供的狀態共享機制，

[click]
讓我們可以在不透過 props 傳遞的情況下，讓深層子組件存取狀態。但它也有一個非常大的副作用：
...
<!--
聽到這邊，相信你已經可以掌握 React 裡所有造成 Re-render 的場景了，

接下來，我們就會正式進入優化的章節了，我們會來聊聊：

1. 有哪些觀察 Re-render 的方法
2. 又有哪些方法可以解決這些 Re-render 問題？包括上面提到 Context 的問題
-->
...
- 請說明 React 中觸發 Re-render 的最根本來源是什麼？
- 為什麼 Hook 不能寫在條件判斷中？請解釋 `hooks` 陣列與 `hookIndex` 在簡化模型中的角色。
- 當 Custom Hook 的 State 改變時，哪些元件會被 Re-render？
- 當 Context 的 value 改變時，哪些元件會被 Re-render？
...
- 檢查現有專案中的 Custom Hook，確認是否存在「引入但未實際使用」的有狀態 Hook。
- 對於純紀錄型資料（例如滾動位置、暫存計算值），優先評估是否可改用 `useRef`。
