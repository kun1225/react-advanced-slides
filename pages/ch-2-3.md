...
<ChapterTitle number="05" subtitle="許多工程師都沒搞懂的" >
Effect 真正意義與陷阱
</ChapterTitle>
...
- 什麼是副作用（Side Effect），舉出至少兩個在 React 中常見的副作用範例。
- 解釋 Pure Function 的兩個核心條件，並說明 React Functional Component 為何被設計為接近 Pure Function。
- 描述 `useEffect` 的實際執行時機（Render Phase 與 Commit Phase 的順序），並說明 Cleanup 發生的時間點。
- 解釋為何「在 Effect 中根據某個 State 去 Set 另一個 State」通常是一種設計味道（design smell）。
...
- 檢查目前專案中所有 `useEffect`，標記並改善那些不是在處理「外部系統」的 Effect。
- 檢視是否有用 `useEffect` 控制 toast、focus、UI 顯示等行為，改為更直接事件驅動寫法。
