...
````md magic-move
```jsx
const MemoComponent = memo(YourComponent, arePropsEqual?)
```
...
const MemoChild = memo(Child, true); // 👈 傳入 true 強制記憶化
```

```jsx
function UserCard({ name, theme }) {
  return (
    <div className={theme}>
      <h3>{name}</h3>
    </div>
...
在第二個參數裡面，可以透過回傳 true、false 來代表 Props 是否改變。

- **true：渲染前後的參數一樣 → 啟動記憶化，不會重新渲染**
- **false：渲染前後的參數不一樣 → 重新渲染組件**

[click]
所以我們可以這樣解決 children 的問題，直接傳入 true 來強制記憶化


[click]
那當然，我個人不是很推薦這樣的寫法，

這第二個參數呢，實務上通常會傳入一個函數，針對特定的 props 來去判斷要不要 re-render

比如這邊有個 UserCard，我們希望只有在 name 改變時被 re-render，就可以這樣寫：
-->
...
- React.memo 的用途為何？
- 為什麼 inline props（物件、函式、JSX、children）會造成 React.memo 失效？
- React.memo 與 useMemo、useCallback 的差異為何？
- 說明 children 與巢狀 memo 組件的陷阱發生原因以及對應解法。
...
- 檢查專案裡所有使用 memo 的組件，確認是否有傳入 inline 物件／函式，若有則改成 `useMemo` 或 `useCallback`。
- 建立團隊規範：避免 inline JSX 或 inline handler 作為 Props 傳遞，推動大家在 code review 時一併檢查。
